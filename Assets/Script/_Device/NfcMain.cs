...
namespace NfcPcSc
{
    public class NfcMain : MonoBehaviour
    {
        ...
        public bool EnableCardReading = false;

        public enum ErrorCode
        {
            SMARTCARD_SERVICE_DISABLED, CANNOT_GET_READER_DATA, NO_READER_AVAILABLE, NO_ERROR = -1
        }

        void Update()
        {
            if (EnableCardReading)
            {
        ...
        public ErrorCode initNfc()
        {
            context = establishContext();
            if(context == IntPtr.Zero)
            {
                return ErrorCode.SMARTCARD_SERVICE_DISABLED;
            }
            readersList = getReaders(context, true);
            if(readersList == new List<string>())
            {
                return ErrorCode.CANNOT_GET_READER_DATA;
            }
            readerStateArray = readerStateChange(context, readersList);
            setState();
        ...
        List<string> getReaders(IntPtr hContext, bool isInit)
        {
            ...
            if (isInit)
            {
                ...
                DataHolder.CardReaderName = readerNameMultiString;
            }
        ...
        ReadResult readCard(IntPtr eContext, string readerName)
        {
            ...
            DataHolder.CardReaderName = readerName;
            disconnect(hCard);
        ...
        {
            if (data[13] != 0x00 || data[14] != 0x3b)
            {
                Debug.Log("FeliCaではありません。");
            }
